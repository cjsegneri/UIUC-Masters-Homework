...
As shown in the Results section, the normality assumption and the constant variance assumptions were not met for the best model we could come up with. Since the assumptions have not been met, this means that we can still "perform" a t-test using `R`, but the results are not valid. The distributions of the parameter estimates will not be what we expect. Hypothesis tests will then accept or reject incorrectly.
